 ...
 #   )
    ...
    if "questions" in st.session_state:
     ...
     #   for i, q in enumerate(st.session_state["questions"]):
        ...
        #        key=f"ans{i}"
        #    )
            ans = st.text_area("your answer", key=f"answer_{i}_{st.session_state['form_reset_counter']}", value="")
            answers.append(ans)
        with st.spinner("Submitting answers and evaluating..."):
            if st.button("Submit Answers"):
                if any(ans.strip() == "" for ans in answers):
                    st.warning("Please answer all questions before submitting.")
                    st.stop()    
            ...
            #   st.write(result["training_plan"])
                training = st.session_state["training_plan"]
                if not training or "error" in training:
                    st.warning("⚠️ Training plan is not available right now. Please try again.")

                    if "error" in training:
                        with st.expander("Show technical details"):
                            st.code(training["error"])
                else:
                    st.success(training.get("summary", "No summary available"))

                    st.write(f"⏳ **Total Estimated Time:** {training['overall_estimated_time_weeks']} weeks")

                    for skill in training.get("training_plan", []):

                        skill_name = skill.get("skill", "Unknown Skill")
                        priority = skill.get("priority", "N/A")

                        with st.expander(f"📚 {skill_name} ({priority} Priority)"):

                            st.write("### Learning Resources")

                            for res in skill.get("learning_resources", []):
                                                st.markdown(f"""
                                **{res.get('type', 'Resource')}**  
                                📖 {res.get('title', '')}  
                                {res.get('description', '')}
                                """)
                            ...
                            for task in skill.get("practice_tasks", []):
                                st.markdown(f"- {task}")

                            st.write(
                                f"⏱ Estimated Time: **{skill.get('estimated_time_weeks', 'N/A')} weeks**"
                            )